...
#   Author      : LiuBo
#   Created date: 2019-04-24 18:40
#   Description :
#
# ================================================================

from core.layers import conv2d, max_pool, avg_pool, fc_connect
from core.blocks import dense_block, transition_block
import tensorflow as tf


class DenseNet(object):
    def __init__(self, dense_block_num=4, growth_rate=32, filter_num=64, reduction=0.0,
                 class_num=10, input_size=224, channels=3, training=True, name="dense_net",
    ...
    def __build_graph(self):
        ...
        x = tf.pad(self.__X, paddings, name="padding1")
        x = conv2d(x, kernel_size=7, stride=2, filter_num=self.filter_num, padding="VALID", name="conv1")
        x = tf.layers.batch_normalization(x, training=self.__is_training)
        x = tf.nn.relu(x)
        paddings = tf.constant([[0, 0], [1, 1], [1, 1], [0, 0]])
        x = tf.pad(x, paddings, name="padding2")
        x = max_pool(x, kernel_size=3, stride=2, padding="VALID", name="max_pool")
